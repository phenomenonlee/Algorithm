...
function solution(n, words) {
    ...
    for (const word of words) {
        ++count;
        if (stack.includes(word)) return [count, order]
        if (stack[stack.length - 1].charAt(stack[stack.length - 1].length - 1) === word.charAt(0)) stack.push(word)
        else return [count, order]
        count === n && (++order, count = 0)
    }
    return [0, 0]
}
